...
import com.bojun.chart.components.YAxis.YAxisLabelPosition;
...
public class YAxisRenderer extends AxisRenderer {
    ...
    protected void drawYLabels(Canvas c, float fixedPosition, float[] positions, float offset) {
        ...
        for (int i = from; i < to; i++) {
            ...
            if (mYAxis.chatType == mYAxis.STRING) {
                c.drawText(text, fixedPosition, positions[i * 2 + 1] + offset, mAxisLabelPaint);
            } else if (mYAxis.chatType == mYAxis.DOUBLE) {
                double t = Double.parseDouble(text);
                c.drawText(String.valueOf(t), fixedPosition, positions[i * 2 + 1] + offset, mAxisLabelPaint);
            }
        }
    }
