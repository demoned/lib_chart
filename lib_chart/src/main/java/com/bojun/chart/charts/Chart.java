...
import com.bojun.chart.animation.ChartAnimator;
import com.bojun.chart.animation.Easing.EasingFunction;
import com.bojun.chart.components.Description;
import com.bojun.chart.components.IMarker;
import com.bojun.chart.components.Legend;
import com.bojun.chart.components.XAxis;
import com.bojun.chart.data.ChartData;
import com.bojun.chart.data.Entry;
import com.bojun.chart.formatter.DefaultValueFormatter;
import com.bojun.chart.formatter.ValueFormatter;
import com.bojun.chart.highlight.ChartHighlighter;
import com.bojun.chart.highlight.Highlight;
import com.bojun.chart.highlight.IHighlighter;
import com.bojun.chart.interfaces.dataprovider.ChartInterface;
import com.bojun.chart.interfaces.datasets.IDataSet;
import com.bojun.chart.listener.ChartTouchListener;
import com.bojun.chart.listener.OnChartGestureListener;
import com.bojun.chart.listener.OnChartValueSelectedListener;
import com.bojun.chart.renderer.DataRenderer;
import com.bojun.chart.renderer.LegendRenderer;
import com.bojun.chart.utils.LogUtil;
import com.bojun.chart.utils.MPPointF;
import com.bojun.chart.utils.Utils;
...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
     ...
     */
    protected Description mDescriptionLeft;

    /**
     * the object responsible for representing the description text
     */
    private Description descriptionRight;
    ...
    protected void init() {
        ...
        mDescriptionLeft = new Description();
    ...
    protected void drawDescription(Canvas c) {
        ...
        if (mDescriptionLeft != null && mDescriptionLeft.isEnabled()) {

            MPPointF position = mDescriptionLeft.getPosition();

            mDescPaint.setTypeface(mDescriptionLeft.getTypeface());
            mDescPaint.setTextSize(mDescriptionLeft.getTextSize());
            mDescPaint.setColor(mDescriptionLeft.getTextColor());
            mDescPaint.setTextAlign(mDescriptionLeft.getTextAlign());

            float x, y;

            // if no position specified, draw on default position
            if (position == null) {
                x = getWidth() - mViewPortHandler.offsetRight() - mDescriptionLeft.getXOffset();
                y = getHeight() - mViewPortHandler.offsetBottom() - mDescriptionLeft.getYOffset();
            } else {
                x = position.x;
                y = position.y;
            }

            c.drawText(mDescriptionLeft.getText(), x, y, mDescPaint);
        }


        // check if description should be drawn
        if (descriptionRight != null && descriptionRight.isEnabled()) {

            MPPointF position = descriptionRight.getPosition();

            mDescPaint.setTypeface(descriptionRight.getTypeface());
            mDescPaint.setTextSize(descriptionRight.getTextSize());
            mDescPaint.setColor(descriptionRight.getTextColor());
            mDescPaint.setTextAlign(descriptionRight.getTextAlign());

            float x, y;

            // if no position specified, draw on default position
            if (position == null) {
                x = getWidth() - mViewPortHandler.offsetRight() - descriptionRight.getXOffset();
                y = getHeight() - mViewPortHandler.offsetBottom() - descriptionRight.getYOffset();
            } else {
                x = position.x;
                y = position.y;
            }
            c.drawText(descriptionRight.getText(), x, y, mDescPaint);
        }
    }
    ...
    public void highlightValue(Highlight high, boolean callListener) {
        ...
        if (high == null)
            ...
            if (mLogEnabled)
                LogUtil.i(LOG_TAG, "Highlighted: " + high.toString());
    ...
    public Highlight getHighlightByTouchPoint(float x, float y) {
        ...
        if (mData == null) {
            LogUtil.e(LOG_TAG, "Can't select by touch. No data set.");
            return null;
        } else
     ...
     * @param desc
     */
    public void setDescriptionLeft(Description desc) {
        this.mDescriptionLeft = desc;
    }
     ...
     * @param desc
     */
    public void setDescriptionRight(Description desc) {
        this.descriptionRight = desc;
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        if (mLogEnabled)
            LogUtil.i(LOG_TAG, "OnSizeChanged()");

        if (w > 0 && h > 0 && w < 10000 && h < 10000) {
            if (mLogEnabled)
                LogUtil.i(LOG_TAG, "Setting chart dimens, width: " + w + ", height: " + h);
            mViewPortHandler.setChartDimens(w, h);
        } else {
            if (mLogEnabled)
                LogUtil.w(LOG_TAG, "*Avoiding* setting chart dimens! width: " + w + ", height: " + h);
        }
